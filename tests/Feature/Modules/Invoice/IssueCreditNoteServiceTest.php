...
it('issues a credit note', function () {
    $invoice = Invoice::factory()->create(['status' => 'open', 'total' => 1000]);
...
it('throws exception if credit note amount exceeds total', function () {
    $invoice = Invoice::factory()->create(['status' => 'open', 'total' => 1000]);
    $service = new IssueCreditNoteService;

    expect(fn () => $service->execute($invoice, 1500, 'Test reason'))
        ->toThrow(DomainException::class, 'Credit note amount cannot exceed invoice total.');
});
