...
use Modules\Notification\Infrastructure\Jobs\SendPaymentFailedEmailJob;
...
test('it sends payment failed email and logs success', function () {
    Mail::fake();
    ...
    $job = new SendPaymentFailedEmailJob($tenant->id, $invoice->id);
    app()->instance(CurrentTenant::class, new CurrentTenant($tenant->id));
    $job->handle();

    Mail::assertSent(PaymentFailedMailable::class, function ($mail) use ($customer, $invoice) {
